...
{/*
    ...
    {
       /* private List<CertificateData> certificateData;
       ...
       [Test, Order(1)]
        public void AddCertification()
        {
            ...
            if (CertificateDataList != null)
            {
                ...
                {
                    Console.WriteLine($"certificate: {item.certificate}, from: {item.from}");
                }
            }

            MarsBaseClass.NavigateToProfileCertification();
            string json = File.ReadAllText(ConstantHelpers.certificateDataPath);
            certificateData = JsonConvert.DeserializeObject<List<CertificateData>>(json);
            
            foreach (CertificateData certificate in certificateData)
            {
